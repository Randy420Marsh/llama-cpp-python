...
class Gemma4ChatHandler(Llava15ChatHandler):
    ...
    # Protects CHAT_FORMAT mutation in __call__ from concurrent requests.
    # Without this, a thinking-mode call that sets self.CHAT_FORMAT between
    # the assignment and super().__call__() can race with a standard call on
    # a shared instance, leaking <|think|> tokens into the standard response.
    _format_lock: threading.Lock = threading.Lock()
    ...
    CHAT_FORMAT = (
        ...
        "<|tool_response>response:{{ message.name | default(message.tool_call_id) }}{{ message.content }}<tool_response|>\n"
        "{% endif %}"
        "{% endfor %}"
        # 6. Generation prompt
        "{% if add_generation_prompt %}"
    ...
    def __call__(self, **kwargs):
        ...
        if enable_thinking:
            ...
            modified_format = original_format
        ...
        # Hold _format_lock for the entire mutation→call→restore cycle so that
        # a concurrent standard request cannot observe the thinking-mode template.
        # The lock is released before post-processing, which doesn't touch
        # CHAT_FORMAT, so contention is minimised.
        with type(self)._format_lock:
            self.CHAT_FORMAT = modified_format
            try:
                result = super().__call__(**kwargs)
            finally:
                # Always restore, even if super().__call__() raises.
